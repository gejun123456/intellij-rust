...
fun ImportCandidate.import(context: RsElement) {
    ...
    val newContext = if (qualifiedNamedItem.item is RsEnumVariant) {
        ...
        val path = psiFactory.tryCreatePath("$enumName::$variantName", RsPsiFactory.PathNamespace.VALUES)!!
        context.replace(path)
        if (processNestedScopesUpwards(path, TYPES_N_VALUES) { it.name == enumName }) return

        val document = path.containingFile.originalFile.viewProvider.document
        if (document != null) {
            PsiDocumentManager.getInstance(path.project).commitDocument(document)
        }
